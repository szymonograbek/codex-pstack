...
At the end of the run, before handing back, check the log told the truth. Read this run's transcript under `${CODEX_HOME:-$HOME/.codex}/sessions/`, selecting the file whose `session_meta.payload.cwd` and opening prompt match this run. Don't read unrelated private chats. Walk the log against what actually happened:
...
Before handing back, spawn an independent `gpt-6-astra/medium` reviewer. Self-review is not a substitute. The subagent reads the audit trail and the run's transcript, then flags what the user should pay attention to. Not a redo of the work, a scan for what's suboptimal or risky.
