...
3. Explore in subagents instructed to read `../references/agents/poteto-agent.md`, with an explicit model per the Subagents section (the **guard-the-context-window** principle skill). Each returns file pointers, conventions, test commands, and entry points. No inlined dumps.
4. Copy the skeleton below into the plan file and fill every placeholder. Unless the operator names a path, write the file under `${CODEX_HOME:-$HOME/.codex}/orchestrate/<project-slug>/docs/`. Keep every heading and every sub-block in the order shown. One section per PR. One PR is one change with its own evidence (the **sequence-verifiable-units** principle skill). Name the execution playbook in **How to read this**. Pick between `playbooks/autopilot-full.md` and `playbooks/autopilot-stack.md` per the rule at the end of `playbooks/autopilot-stack.md`. A standing program takes `playbooks/orchestrate.md`.
...
The program runs `<poteto-mode-directory>/playbooks/<execution playbook>.md`. <Who merges, and which PR ids are the operator's items that stop at merge-ready.>
...
- [ ] Read these from the installed skills at program start. Re-read them at every tick.
  - [ ] `<poteto-mode-directory>/playbooks/<execution playbook>.md`
  - [ ] `<installed swarm SKILL.md>`
  - [ ] `<installed control skill SKILL.md>`
  - [ ] `<poteto-mode-directory>/playbooks/opening-a-pr.md`
  - [ ] `<installed leaf skill SKILL.md>`
- [ ] Arm the 30-minute audit tick with Codex's recurring monitoring mechanism. Never leave the cadence to memory.
...
- [ ] At the merge-ready head SHA, run the swarm per `<installed swarm SKILL.md>`. One gates lane. The ten live lanes from the PR's **Verify, live** block. The perf lane from its **Verify, perf** block. One audit lane that reads the diff and the receipts and distrusts the PR body.
...
**Verify, live.** Tests alone are not sufficient verification. A PR is verified only when its unit, live, and perf boxes are all checked. Ten lanes on `gpt-5.6-sol/medium` at the PR head, per the boot recipe.
...
<Docs to read before editing. Which PRs get `<installed how SKILL.md>` and `<installed interrogate SKILL.md>`. The trail per `<installed show-me-your-work SKILL.md>`.>
````
