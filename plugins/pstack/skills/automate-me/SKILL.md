...
This skill orchestrates three others: an inline mining pass (see step 1), Codex's built-in **skill-creator** skill (authoring), and the **unslop** skill (prose discipline). It sequences them. It doesn't replace them.

## Flow

### 0. Check for an existing skill

Look recursively for `.agents/skills/**/*-mode/SKILL.md` and `~/.agents/skills/*-mode/SKILL.md` matching the user's handle. Mode skills can live in a personal category directory (`.agents/skills/<handle>/`), not only at the top level. If one exists, confirm intent with structured user input when available (unless they already said "update my skill" or similar):
...
Locate the active workspace's transcripts before fanning out. Codex stores them under `${CODEX_HOME:-$HOME/.codex}/sessions/`. Select only transcripts whose `session_meta.payload.cwd` matches the active workspace. Don't read private chats from unrelated projects.
...
Mining misses intent that hasn't come up yet. Use structured user input when available rather than asking the user to type from scratch.
