...
Transcripts live under `${CODEX_HOME:-$HOME/.codex}/sessions/`, nested by date. Each transcript's first `session_meta` item names its `payload.cwd`; every line is one event.
