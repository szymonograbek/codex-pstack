...
- i want PStack available in the Codex execution environment for shared dependencies such as `how`, `why`, `tdd`, `unslop`, and the required principle skills.
...
Read [setup](skills/setup-benny/SKILL.md) directly. Use the directory containing this file as the source pack. The user can also enter through `$pstack:setup-benny`.

Setup copies this pack to the target repository, resolves the configured adapters, verifies OpenAI models, and prepares two paused Codex jobs. Read [scheduling](references/scheduling.md) before choosing polling. The operational files remain direct instructions, outside the registered skill root.

Installing or browsing the pack does not activate jobs or authorize external writes. Obtain the required project and channel configuration when the user asks to set it up.
